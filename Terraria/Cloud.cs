	...
	public class Cloud
	{
		...
		public static void addCloud()
		{
			...
			if (num >= 0)
			{
				...
				if (num2 > 0f)
				{
					Main.cloud[num].position.X = -(float)Main.cloud[num].width - (float)Main.cloudTexture[Main.cloud[num].type].Width - (float)Cloud.rand.Next(Main.screenWidth * 2);
				}
				else
				{
				...
				cloud.position.Y = cloud.position.Y - (float)Cloud.rand.Next((int)((float)Main.screenHeight * 0.15f));
				Cloud cloud2 = Main.cloud[num];
				cloud2.position.Y = cloud2.position.Y - (float)Cloud.rand.Next((int)((float)Main.screenHeight * 0.15f));
		...
		public void Update()
		{
			...
			else if (this.position.X + (float)this.width + (float)Main.cloudTexture[this.type].Width < -200f)
			{
				this.active = false;
			}
