...
namespace Terraria
{
	internal class WorldGen
	{
		...
		public static bool MoveNPC(int x, int y, int n)
		{
			...
			{
				Main.NewText("目前这不是一个有效的居住空间。", 255, 240, 20);
				result = false;
			}
			else if (!WorldGen.RoomNeeds(WorldGen.spawnNPC))
			{
				...
				Main.NewText("房间目前还差" + text + "，就可以入住了~", 255, 240, 20);
				result = false;
			}
			else
			{
				...
				if (WorldGen.hiScore <= 0)
				{
					...
					{
						Main.NewText("房间已经住人了。", 255, 240, 20);
					}
					else if (WorldGen.roomEvil)
					{
						Main.NewText("房间已经被腐化了。", 255, 240, 20);
					}
					else
					{
						Main.NewText("目前这不是一个符合尺寸的居住空间。", 255, 240, 20);
					}
					result = false;
				}
				else
					...
					if (WorldGen.RoomNeeds(WorldGen.spawnNPC))
					{
						...
						if (WorldGen.hiScore > 0)
						{
							...
							if (num == -1)
							{
								...
								string str = Main.npc[num7].CName;
								if (Main.chrName[Main.npc[num7].type] != "")
								{
									str = NPC.FindCNames(Main.chrName[Main.npc[num7].type]) + "(" + Main.npc[num7].CName + ")";
								}
								if (Main.netMode == 0)
								{
									Main.NewText(str + " 已抵达！", 50, 125, 255);
								}
								else if (Main.netMode == 2)
								{
									NetMessage.SendData(25, -1, -1, str + " 已抵达！", 255, 50f, 125f, 255f, 0);
								}
							}
							else
							{
		...
		public static void ScoreRoom(int ignoreNPC = -1)
		{
			...
			for (int n = num2 + 1; n < num3; n++)
			{
				for (int num7 = num4 + 2; num7 < num5 + 2; num7++)
				{
					if (Main.tile[n, num7].active)
					{
						int num8 = num6;
						if (Main.tileSolid[(int)Main.tile[n, num7].type] && !Main.tileSolidTop[(int)Main.tile[n, num7].type] && !Collision.SolidTiles(n - 1, n + 1, num7 - 3, num7 - 1) && Main.tile[n - 1, num7].active && Main.tileSolid[(int)Main.tile[n - 1, num7].type] && Main.tile[n + 1, num7].active && Main.tileSolid[(int)Main.tile[n + 1, num7].type])
						{
		...
		public static bool meteor(int i, int j)
		{
			...
			else if (j < 50 || j > Main.maxTilesY - 50)
			{
				result = false;
			}
			else
			{
				...
				if (Main.netMode == 0)
				{
					Main.NewText("一颗陨石砸了下来！", 50, 255, 130);
				}
				else if (Main.netMode == 2)
				{
					NetMessage.SendData(25, -1, -1, "一颗陨石砸了下来！", 255, 50f, 255f, 130f, 0);
				}
				if (Main.netMode != 1)
				{
			...
			if (WorldGen.loadFailed || !WorldGen.loadSuccess)
			{
				...
				if (WorldGen.loadFailed || !WorldGen.loadSuccess)
				{
					...
					else
					{
						if (!WorldGen.worldBackup)
						{
							Console.WriteLine("载入失败！没找到备份文件...");
							return;
						}
		...
		public static void clearWorld()
		{
			...
			if (Main.netMode == 1 || WorldGen.lastMaxTilesX > Main.maxTilesX || WorldGen.lastMaxTilesY > Main.maxTilesY)
			{
				for (int i = 0; i < WorldGen.lastMaxTilesX; i++)
				{
					...
					Main.statusText = "释放闲置资源: " + (int)(num * 100f + 1f) + "%";
			...
			if (Main.netMode != 1)
			{
				for (int k = 0; k < Main.maxTilesX; k++)
				{
					...
					Main.statusText = "重置游戏对象: " + (int)(num2 * 100f + 1f) + "%";
							...
							using (FileStream fileStream = new FileStream(text, FileMode.Create))
							{
								using (BinaryWriter binaryWriter = new BinaryWriter(fileStream))
								{
									...
									for (int i = 0; i < Main.maxTilesX; i++)
									{
										...
										Main.statusText = "保存地图数据: " + (int)(num * 100f + 1f) + "%";
					...
					try
					{
						...
						else
						{
							...
							for (int j = 0; j < Main.maxTilesX; j++)
							{
								...
								Main.statusText = "载入世界数据: " + (int)(num2 * 100f + 1f) + "%";
							...
							if (!WorldGen.loadFailed && WorldGen.loadSuccess)
							{
								...
								for (int num9 = 0; num9 < Main.maxTilesX; num9++)
								{
									...
									Main.statusText = "检查物块校准: " + (int)(num10 * 100f + 1f) + "%";
								...
								while (Liquid.numLiquid > 0 && num11 < 100000)
								{
									...
									Main.statusText = "稳定液体水平面: " + (int)(num14 * 100f / 2f + 50f) + "%";
									Liquid.UpdateLiquid();
								}
		...
		public static void generateWorld(int seed = -1)
		{
			...
			for (int i = 0; i < Main.maxTilesX; i++)
			{
				...
				Main.statusText = "生成世界地形: " + (int)(num10 * 100f + 1f) + "%";
				if (num3 < num5)
				{
					num5 = num3;
				}
			...
			for (int num43 = 1; num43 < Main.maxTilesX - 1; num43++)
			{
				...
				Main.statusText = "泥土后面贴墙: " + (int)(num44 * 100f + 1f) + "%";
				bool flag4 = false;
			...
			for (int num57 = 0; num57 < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 0.0015); num57++)
			{
				...
				Main.statusText = "随机创建地下洞穴: " + (int)(num58 * 100f + 1f) + "%";
				int type = -1;
			...
			for (int num59 = 0; num59 < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 3E-05); num59++)
			{
				...
				Main.statusText = "生成小的洞窟: " + (int)(num60 * 100f + 1f) + "%";
			...
			for (int num61 = 0; num61 < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 0.00013); num61++)
			{
				...
				Main.statusText = "生成大的洞窟: " + (int)(num62 * 100f + 1f) + "%";
				if (num8 <= (double)Main.maxTilesY)
				{
					int type3 = -1;
			...
			Main.statusText = "生成丛林: 0%";
			float num76 = (float)(Main.maxTilesX / 4200);
			num76 *= 1.5f;
			float num77 = (float)WorldGen.genRand.Next(15, 30) * 0.01f;
			int num78;
			if (num9 == -1)
			{
				num77 = 1f - num77;
				num78 = (int)((float)Main.maxTilesX * num77);
			}
			else
			{
				num78 = (int)((float)Main.maxTilesX * num77);
			...
			num78 += WorldGen.genRand.Next((int)(-100f * num76), (int)(101f * num76));
			num79 += WorldGen.genRand.Next((int)(-100f * num76), (int)(101f * num76));
			int num80 = num78;
			int num81 = num79;
			WorldGen.TileRunner(num78, num79, (double)WorldGen.genRand.Next((int)(250f * num76), (int)(500f * num76)), WorldGen.genRand.Next(50, 150), 59, false, (float)(num9 * 3), 0f, false, true);
			int num82 = 0;
			while ((float)num82 < 6f * num76)
			{
				WorldGen.TileRunner(num78 + WorldGen.genRand.Next(-(int)(125f * num76), (int)(125f * num76)), num79 + WorldGen.genRand.Next(-(int)(125f * num76), (int)(125f * num76)), (double)WorldGen.genRand.Next(3, 7), WorldGen.genRand.Next(3, 8), WorldGen.genRand.Next(63, 65), false, 0f, 0f, false, true);
				num82++;
			}
			WorldGen.mudWall = true;
			Main.statusText = "生成丛林: 15%";
			num78 += WorldGen.genRand.Next((int)(-250f * num76), (int)(251f * num76));
			num79 += WorldGen.genRand.Next((int)(-150f * num76), (int)(151f * num76));
			int num83 = num78;
			int num84 = num79;
			int num85 = num78;
			int num86 = num79;
			WorldGen.TileRunner(num78, num79, (double)WorldGen.genRand.Next((int)(250f * num76), (int)(500f * num76)), WorldGen.genRand.Next(50, 150), 59, false, 0f, 0f, false, true);
			WorldGen.mudWall = false;
			int num87 = 0;
			while ((float)num87 < 6f * num76)
			{
				WorldGen.TileRunner(num78 + WorldGen.genRand.Next(-(int)(125f * num76), (int)(125f * num76)), num79 + WorldGen.genRand.Next(-(int)(125f * num76), (int)(125f * num76)), (double)WorldGen.genRand.Next(3, 7), WorldGen.genRand.Next(3, 8), WorldGen.genRand.Next(65, 67), false, 0f, 0f, false, true);
				num87++;
			}
			WorldGen.mudWall = true;
			Main.statusText = "生成丛林: 30%";
			num78 += WorldGen.genRand.Next((int)(-400f * num76), (int)(401f * num76));
			num79 += WorldGen.genRand.Next((int)(-150f * num76), (int)(151f * num76));
			int num88 = num78;
			int num89 = num79;
			WorldGen.TileRunner(num78, num79, (double)WorldGen.genRand.Next((int)(250f * num76), (int)(500f * num76)), WorldGen.genRand.Next(50, 150), 59, false, (float)(num9 * -3), 0f, false, true);
			...
			{
				WorldGen.TileRunner(num78 + WorldGen.genRand.Next(-(int)(125f * num76), (int)(125f * num76)), num79 + WorldGen.genRand.Next(-(int)(125f * num76), (int)(125f * num76)), (double)WorldGen.genRand.Next(3, 7), WorldGen.genRand.Next(3, 8), WorldGen.genRand.Next(67, 69), false, 0f, 0f, false, true);
				num90++;
			}
			WorldGen.mudWall = true;
			Main.statusText = "生成丛林: 45%";
			num78 = (num80 + num83 + num88) / 3;
			num79 = (num81 + num84 + num89) / 3;
			WorldGen.TileRunner(num78, num79, (double)WorldGen.genRand.Next((int)(400f * num76), (int)(600f * num76)), 10000, 59, false, 0f, -20f, true, true);
			WorldGen.JungleRunner(num78, num79);
			Main.statusText = "生成丛林: 60%";
			WorldGen.mudWall = false;
			for (int num91 = 0; num91 < Main.maxTilesX / 10; num91++)
			{
				num78 = WorldGen.genRand.Next(20, Main.maxTilesX - 20);
				num79 = WorldGen.genRand.Next((int)Main.rockLayer, Main.maxTilesY - 200);
				while (Main.tile[num78, num79].wall != 15)
				{
					num78 = WorldGen.genRand.Next(20, Main.maxTilesX - 20);
					num79 = WorldGen.genRand.Next((int)Main.rockLayer, Main.maxTilesY - 200);
				}
				WorldGen.MudWallRunner(num78, num79);
			}
			num78 = num85;
			num79 = num86;
			int num92 = 0;
			...
			{
				Main.statusText = "生成丛林: " + (int)(60f + (float)num92 / num76) + "%";
				num78 += WorldGen.genRand.Next((int)(-5f * num76), (int)(6f * num76));
				num79 += WorldGen.genRand.Next((int)(-5f * num76), (int)(6f * num76));
				WorldGen.TileRunner(num78, num79, (double)WorldGen.genRand.Next(40, 100), WorldGen.genRand.Next(300, 500), 59, false, 0f, 0f, false, true);
				num92++;
			}
			int num93 = 0;
			while ((float)num93 <= 10f * num76)
			{
				Main.statusText = "生成丛林: " + (int)(80f + (float)num93 / num76 * 2f) + "%";
				num78 = num85 + WorldGen.genRand.Next((int)(-600f * num76), (int)(600f * num76));
				num79 = num86 + WorldGen.genRand.Next((int)(-200f * num76), (int)(200f * num76));
				while (num78 < 1 || num78 >= Main.maxTilesX - 1 || num79 < 1 || num79 >= Main.maxTilesY - 1 || Main.tile[num78, num79].type != 59)
				{
					num78 = num85 + WorldGen.genRand.Next((int)(-600f * num76), (int)(600f * num76));
					num79 = num86 + WorldGen.genRand.Next((int)(-200f * num76), (int)(200f * num76));
				}
				...
				{
					num78 += WorldGen.genRand.Next(-30, 31);
					...
					WorldGen.TileRunner(num78, num79, (double)WorldGen.genRand.Next(10, 20), WorldGen.genRand.Next(30, 70), type4, false, 0f, 0f, false, true);
					num94++;
				}
				num93++;
			}
			...
			while ((float)num95 <= 300f * num76)
			{
				...
				WorldGen.TileRunner(num78, num79, (double)WorldGen.genRand.Next(4, 10), WorldGen.genRand.Next(5, 30), 1, false, 0f, 0f, false, true);
				if (WorldGen.genRand.Next(4) == 0)
				{
					int type5 = WorldGen.genRand.Next(63, 69);
					WorldGen.TileRunner(num78 + WorldGen.genRand.Next(-1, 2), num79 + WorldGen.genRand.Next(-1, 2), (double)WorldGen.genRand.Next(3, 7), WorldGen.genRand.Next(4, 8), type5, false, 0f, 0f, false, true);
				}
				num95++;
			}
			...
			while ((float)num98 < num96)
			{
				...
				while (flag5)
				{
					...
					if (Main.tile[num78, num79].type == 59)
					{
						flag5 = false;
						int num99 = WorldGen.genRand.Next(2, 4);
						int num100 = WorldGen.genRand.Next(2, 4);
						for (int num101 = num78 - num99 - 1; num101 <= num78 + num99 + 1; num101++)
						{
						...
						for (int num103 = num78 - num99; num103 <= num78 + num99; num103++)
						{
						...
						{
							num105++;
							int num106 = WorldGen.genRand.Next(num78 - num99, num78 + num99 + 1);
						...
						for (int num108 = num78 - num99 - 1; num108 <= num78 + num99 + 1; num108++)
						{
						...
						for (int num110 = num78 - num99 - 1; num110 <= num78 + num99 + 1; num110++)
						{
						...
						for (int num112 = num78 - num99 - 1; num112 <= num78 + num99 + 1; num112++)
						{
							int num113 = 4;
						...
						while (num99 > -1)
						{
							for (int num116 = num78 - num99 - 1; num116 <= num78 + num99 + 1; num116++)
							{
						...
						WorldGen.JChestX[WorldGen.numJChests] = num78;
			...
			for (int num156 = 0; num156 < Main.maxTilesX; num156++)
			{
				...
				Main.statusText = "创建地下世界: " + (int)(num157 * 100f / 2f + 50f) + "%";
			...
			for (int num164 = 0; num164 < num163; num164++)
			{
				...
				Main.statusText = "添加水体: " + (int)(num165 * 100f) + "%";
			...
			while ((double)num168 < (double)Main.maxTilesX * 0.00045)
			{
				...
				Main.statusText = "生成世界BOSS: " + (int)(num169 * 100f) + "%";
				bool flag9 = false;
				int num170 = 0;
				int num171 = 0;
				int num172 = 0;
			...
			Main.statusText = "生成分支...";
			for (int num200 = 0; num200 < 2; num200++)
			{
				if (num200 == 0)
				{
					int num201 = 0;
					int num202 = WorldGen.genRand.Next(125, 200) + 50;
					if (num9 == 1)
					{
						num202 = 275;
					}
				...
				{
					int num201 = Main.maxTilesX - WorldGen.genRand.Next(125, 200) - 50;
					int num202 = Main.maxTilesX;
					if (num9 == -1)
					{
			...
			for (int num228 = 0; num228 < Main.maxTilesX; num228++)
			{
				...
				Main.statusText = "重力作用沙子: " + (int)(num229 * 100f) + "%";
				for (int num230 = Main.maxTilesY - 5; num230 > 0; num230--)
				{
					if (Main.tile[num228, num230].active)
					{
						...
						else if (Main.tile[num228, num230].type == 123)
						{
							int num232 = num230;
			...
			for (int num233 = 3; num233 < Main.maxTilesX - 3; num233++)
			{
				...
				Main.statusText = "清理多余的土壤背景: " + (int)(num234 * 100f + 1f) + "%";
				bool flag11 = true;
				int num235 = 0;
				while ((double)num235 < Main.worldSurface)
				{
					...
					else if (Main.tile[num233, num235].wall == 0 && Main.tile[num233, num235 + 1].wall == 0 && Main.tile[num233, num235 + 2].wall == 0 && Main.tile[num233, num235 + 3].wall == 0 && Main.tile[num233, num235 + 4].wall == 0 && Main.tile[num233 - 1, num235].wall == 0 && Main.tile[num233 + 1, num235].wall == 0 && Main.tile[num233 - 2, num235].wall == 0 && Main.tile[num233 + 2, num235].wall == 0 && !Main.tile[num233, num235].active && !Main.tile[num233, num235 + 1].active && !Main.tile[num233, num235 + 2].active && !Main.tile[num233, num235 + 3].active)
					{
						flag11 = true;
					}
					num235++;
			...
			for (int num236 = 0; num236 < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 2E-05); num236++)
			{
				...
				Main.statusText = "安放祭坛: " + (int)(num237 * 100f + 1f) + "%";
				bool flag12 = false;
				int num238 = 0;
				while (!flag12)
				{
			...
			while (num246 < 10)
			{
				...
				while (Liquid.numLiquid > 0)
				{
					...
					if (num246 == 1)
					{
						Main.statusText = "稳定液体水平面: " + (int)(num249 * 100f / 3f + 33f) + "%";
					}
					int num250 = 10;
					if (num246 <= num250)
					{
				...
				WorldGen.WaterCheck();
				Main.statusText = "稳定液体水平面: " + (int)((float)num246 * 10f / 3f + 66f) + "%";
			}
			Liquid.quickSettle = false;
			...
			for (int num252 = 0; num252 < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 2E-05); num252++)
			{
				...
				Main.statusText = "安放生命水晶: " + (int)(num253 * 100f + 1f) + "%";
				bool flag13 = false;
				int num254 = 0;
				while (!flag13)
				{
			...
			while ((float)num256 < 82f * num251)
			{
				...
				Main.statusText = "安放雕像: " + (int)(num257 * 100f + 1f) + "%";
				bool flag14 = false;
				int num258 = 0;
				while (!flag14)
				{
			...
			for (int num261 = 0; num261 < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 1.6E-05); num261++)
			{
				...
				Main.statusText = "隐藏财宝: " + (int)(num262 * 100f + 1f) + "%";
				bool flag15 = false;
				int num263 = 0;
				while (!flag15)
				{
			...
			for (int num267 = 0; num267 < (int)((double)Main.maxTilesX * 0.005); num267++)
			{
				...
				Main.statusText = "隐藏更多的财宝: " + (int)(num268 * 100f + 1f) + "%";
				bool flag16 = false;
				int num269 = 0;
				while (!flag16)
				{
			...
			for (int num273 = 0; num273 < WorldGen.numJChests; num273++)
			{
				...
				Main.statusText = "隐藏丛林财宝: " + (int)(num274 * 100f + 1f) + "%";
				num272++;
				int contain = 211;
				if (num272 == 1)
				{
			...
			while ((float)num278 < 9f * num251)
			{
				...
				Main.statusText = "隐藏水下财宝: " + (int)(num279 * 100f + 1f) + "%";
				num277++;
				int contain2;
				if (num277 == 1)
				{
			...
			for (int num283 = 0; num283 < (int)((double)Main.maxTilesX * 0.05); num283++)
			{
				...
				Main.statusText = "安放陷阱: " + (int)(num284 * 100f + 1f) + "%";
			...
			for (int num288 = 0; num288 < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 0.0008); num288++)
			{
				...
				Main.statusText = "安放可打碎物品: " + (int)(num289 * 100f + 1f) + "%";
				bool flag19 = false;
				int num290 = 0;
				while (!flag19)
				{
			...
			for (int num294 = 0; num294 < Main.maxTilesX / 200; num294++)
			{
				...
				Main.statusText = "安放地狱熔炉: " + (int)(num295 * 100f + 1f) + "%";
				bool flag21 = false;
				int num296 = 0;
				while (!flag21)
				{
			...
			for (int num299 = 0; num299 < Main.maxTilesX; num299++)
			{
				...
				while ((double)num300 < Main.worldSurface - 1.0)
				{
					...
					else if (Main.tile[num56, num300].wall == 0)
					{
						flag22 = true;
					}
					num300++;
				...
				if (WorldGen.genRand.Next(8) == 0)
				{
					...
					while ((double)num302 < Main.worldSurface - 1.0)
					{
						if (Main.tile[num301, num302].active && Main.tile[num301, num302].type == 53 && !Main.tile[num301, num302 - 1].active && Main.tile[num301, num302 - 1].wall == 0)
						{
							...
							else if (num301 > 400 && num301 < Main.maxTilesX - 400)
							{
			...
			Main.statusText = "种植向日葵...";
			int num310 = 0;
			while ((double)num310 < (double)Main.maxTilesX * 0.002)
			{
				int num311 = Main.maxTilesX / 2;
				int num312 = WorldGen.genRand.Next(Main.maxTilesX);
				int num313 = num312 - WorldGen.genRand.Next(10) - 7;
				int num314 = num312 + WorldGen.genRand.Next(10) + 7;
				if (num313 < 0)
				{
					num313 = 0;
				}
				...
				for (int num315 = num313; num315 < num314; num315++)
				{
					int num316 = 1;
					while ((double)num316 < Main.worldSurface - 1.0)
					{
						if (Main.tile[num315, num316].type == 2 && Main.tile[num315, num316].active && !Main.tile[num315, num316 - 1].active)
						{
							WorldGen.PlaceTile(num315, num316 - 1, 27, true, false, -1, 0);
						}
			...
			Main.statusText = "植树...";
			int num317 = 0;
			while ((double)num317 < (double)Main.maxTilesX * 0.003)
			{
				int num318 = WorldGen.genRand.Next(50, Main.maxTilesX - 50);
				int num319 = WorldGen.genRand.Next(25, 50);
				for (int num320 = num318 - num319; num320 < num318 + num319; num320++)
				{
					int num321 = 20;
					while ((double)num321 < Main.worldSurface)
					{
						WorldGen.GrowEpicTree(num320, num321);
						num321++;
					}
				}
				num317++;
			...
			WorldGen.AddTrees();
			Main.statusText = "种植草药...";
			int num322 = 0;
			while ((double)num322 < (double)Main.maxTilesX * 1.7)
			{
				WorldGen.PlantAlch();
				num322++;
			}
			...
			{
				for (int num324 = 0; num324 < Main.maxTilesY; num324++)
				{
					if (Main.tile[num323, num324].active)
					{
						if (num324 >= (int)Main.worldSurface && Main.tile[num323, num324].type == 70 && !Main.tile[num323, num324 - 1].active)
						{
							WorldGen.GrowShroom(num323, num324);
							if (!Main.tile[num323, num324 - 1].active)
							{
								WorldGen.PlaceTile(num323, num324 - 1, 71, true, false, -1, 0);
							}
						}
						if (Main.tile[num323, num324].type == 60 && !Main.tile[num323, num324 - 1].active)
						{
							WorldGen.PlaceTile(num323, num324 - 1, 61, true, false, -1, 0);
						}
					}
				}
			}
			Main.statusText = "长出藤蔓...";
			for (int num325 = 0; num325 < Main.maxTilesX; num325++)
			{
				int num326 = 0;
				int num327 = 0;
				while ((double)num327 < Main.worldSurface)
				{
					if (num326 > 0 && !Main.tile[num325, num327].active)
					{
						Main.tile[num325, num327].active = true;
						Main.tile[num325, num327].type = 52;
						num326--;
					}
					else
					{
					...
					if (Main.tile[num325, num327].active && Main.tile[num325, num327].type == 2 && WorldGen.genRand.Next(5) < 3)
					{
				...
				num326 = 0;
				for (int num328 = 0; num328 < Main.maxTilesY; num328++)
				{
					if (num326 > 0 && !Main.tile[num325, num328].active)
					{
						Main.tile[num325, num328].active = true;
						Main.tile[num325, num328].type = 62;
						num326--;
					}
					else
					{
					...
					if (Main.tile[num325, num328].active && Main.tile[num325, num328].type == 60 && WorldGen.genRand.Next(5) < 3)
					{
						num326 = WorldGen.genRand.Next(1, 10);
					}
				}
			...
			Main.statusText = "种花...";
			int num329 = 0;
			while ((double)num329 < (double)Main.maxTilesX * 0.005)
			{
				int num330 = WorldGen.genRand.Next(20, Main.maxTilesX - 20);
				int num331 = WorldGen.genRand.Next(5, 15);
				int num332 = WorldGen.genRand.Next(15, 30);
				int num333 = 1;
				while ((double)num333 < Main.worldSurface - 1.0)
				{
					if (Main.tile[num330, num333].active)
					{
						for (int num334 = num330 - num331; num334 < num330 + num331; num334++)
						{
							for (int num335 = num333 - num332; num335 < num333 + num332; num335++)
							{
								if (Main.tile[num334, num335].type == 3 || Main.tile[num334, num335].type == 24)
								{
									Main.tile[num334, num335].frameX = (short)(WorldGen.genRand.Next(6, 8) * 18);
								}
							}
						}
						break;
			...
			Main.statusText = "种蘑菇...";
			int num336 = 0;
			while ((double)num336 < (double)Main.maxTilesX * 0.002)
			{
				int num337 = WorldGen.genRand.Next(20, Main.maxTilesX - 20);
				int num338 = WorldGen.genRand.Next(4, 10);
				int num339 = WorldGen.genRand.Next(15, 30);
				int num340 = 1;
				while ((double)num340 < Main.worldSurface - 1.0)
				{
					if (Main.tile[num337, num340].active)
					{
						for (int num341 = num337 - num338; num341 < num337 + num338; num341++)
						{
							for (int num342 = num340 - num339; num342 < num340 + num339; num342++)
							{
								if (Main.tile[num341, num342].type == 3 || Main.tile[num341, num342].type == 24)
								{
									Main.tile[num341, num342].frameX = 144;
								}
							}
						}
						break;
		...
		public static void smCallBack(object threadContext)
		{
			if (!Main.hardMode)
			{
				...
				WorldGen.GERunner(i2, 0, 3f * -(float)num2, 5f, false);
				if (Main.netMode == 0)
				{
					Main.NewText("远古的光明之魂与黑暗之魂被释放了！", 50, 255, 130);
				}
				else if (Main.netMode == 2)
				{
					NetMessage.SendData(25, -1, -1, "远古的光明之魂与黑暗之魂被释放了！", 255, 50f, 255f, 130f, 0);
				}
				if (Main.netMode == 2)
				{
		...
		public static bool AddLifeCrystal(int i, int j)
		{
			...
			for (int k = j; k < Main.maxTilesY; k++)
			{
				if (Main.tile[i, k].active && Main.tileSolid[(int)Main.tile[i, k].type])
				{
					...
					else if (!WorldGen.EmptyTileCheck(i - 1, i, num - 1, num, -1))
					{
						result = false;
					}
					else
		...
		public static void MakeDungeon(int x, int y, int tileType = 41, int wallType = 7)
		{
			...
			while (num3 > 0f)
			{
				...
				num3 -= 1f;
				Main.statusText = "创建地牢: " + (int)((num4 - num3) / num4 * 60f) + "%";
				if (num5 > 0)
				{
					num5--;
				}
			...
			while (num14 < Main.maxTilesX / 20)
			{
				...
				if ((int)Main.tile[num67, num68].wall == wallType && !Main.tile[num67, num68].active)
				{
					...
					while (flag5 && !Main.tile[num67, num68].active)
					{
						...
						else if (Main.tile[num67, num68].active && !Main.tileDungeon[(int)Main.tile[num67, num68].type])
						{
							flag5 = false;
						}
					}
		...
		public static bool OpenDoor(int i, int j, int direction)
		{
			...
			if (flag)
			{
				...
				Main.tile[num3, num].active = true;
				Main.tile[num3, num].type = 11;
				Main.tile[num3, num].frameY = 0;
				Main.tile[num3, num].frameX = num2;
				if (Main.tile[num3 + 1, num] == null)
				{
					Main.tile[num3 + 1, num] = new Tile();
				}
				Main.tile[num3 + 1, num].active = true;
				Main.tile[num3 + 1, num].type = 11;
				Main.tile[num3 + 1, num].frameY = 0;
				Main.tile[num3 + 1, num].frameX = (byte)(num2 + 18);
				if (Main.tile[num3, num + 1] == null)
				{
					Main.tile[num3, num + 1] = new Tile();
				}
				Main.tile[num3, num + 1].active = true;
				Main.tile[num3, num + 1].type = 11;
				Main.tile[num3, num + 1].frameY = 18;
				Main.tile[num3, num + 1].frameX = num2;
				if (Main.tile[num3 + 1, num + 1] == null)
				{
					Main.tile[num3 + 1, num + 1] = new Tile();
				}
				Main.tile[num3 + 1, num + 1].active = true;
				Main.tile[num3 + 1, num + 1].type = 11;
				Main.tile[num3 + 1, num + 1].frameY = 18;
				Main.tile[num3 + 1, num + 1].frameX = (byte)(num2 + 18);
				if (Main.tile[num3, num + 2] == null)
				{
					Main.tile[num3, num + 2] = new Tile();
				}
				Main.tile[num3, num + 2].active = true;
				Main.tile[num3, num + 2].type = 11;
				Main.tile[num3, num + 2].frameY = 36;
				Main.tile[num3, num + 2].frameX = num2;
				if (Main.tile[num3 + 1, num + 2] == null)
				{
					Main.tile[num3 + 1, num + 2] = new Tile();
				}
				Main.tile[num3 + 1, num + 2].active = true;
				Main.tile[num3 + 1, num + 2].type = 11;
				Main.tile[num3 + 1, num + 2].frameY = 36;
				Main.tile[num3 + 1, num + 2].frameX = (byte)(num2 + 18);
				for (int n = num3 - 1; n <= num3 + 2; n++)
				{
		...
		public static void Check2xX(int i, int j, byte type)
		{
			if (!WorldGen.destroyObject)
			{
				...
				for (int l = 0; l < num3; l++)
				{
					...
					else if (Main.tile[num, num2 + l].type != type)
					{
						flag = true;
					}
					else if ((int)Main.tile[num, num2 + l].frameY != l * 18)
					{
						flag = true;
					}
					else if ((int)Main.tile[num, num2 + l].frameX != frameX)
					{
						flag = true;
					}
					...
					else if (Main.tile[num + 1, num2 + l].type != type)
					{
						flag = true;
					}
					else if ((int)Main.tile[num + 1, num2 + l].frameY != l * 18)
					{
						flag = true;
					}
					else if ((int)Main.tile[num + 1, num2 + l].frameX != frameX + 18)
					{
						flag = true;
					}
				}
						...
						if (num3 >= 0)
						{
							if (num4 <= Main.maxTilesY)
							{
								...
								if (!flag)
								{
									...
									else if (Main.tile[num6, num7 + 2].active && Main.tileSolid[(int)Main.tile[num6, num7 + 2].type] && Main.tile[num6 + 1, num7 + 2].active && Main.tileSolid[(int)Main.tile[num6 + 1, num7 + 2].type])
									{
										num8 = 0;
									}
									else if (Main.tile[num6, num7 - 1].active && Main.tileSolid[(int)Main.tile[num6, num7 - 1].type] && !Main.tileSolidTop[(int)Main.tile[num6, num7 - 1].type] && Main.tile[num6 + 1, num7 - 1].active && Main.tileSolid[(int)Main.tile[num6 + 1, num7 - 1].type] && !Main.tileSolidTop[(int)Main.tile[num6 + 1, num7 - 1].type])
									{
										num8 = 1;
									}
									else if (Main.tile[num6 - 1, num7].active && Main.tileSolid[(int)Main.tile[num6 - 1, num7].type] && !Main.tileSolidTop[(int)Main.tile[num6 - 1, num7].type] && Main.tile[num6 - 1, num7 + 1].active && Main.tileSolid[(int)Main.tile[num6 - 1, num7 + 1].type] && !Main.tileSolidTop[(int)Main.tile[num6 - 1, num7 + 1].type])
									{
										num8 = 2;
									}
									else if (Main.tile[num6 + 2, num7].active && Main.tileSolid[(int)Main.tile[num6 + 2, num7].type] && !Main.tileSolidTop[(int)Main.tile[num6 + 2, num7].type] && Main.tile[num6 + 2, num7 + 1].active && Main.tileSolid[(int)Main.tile[num6 + 2, num7 + 1].type] && !Main.tileSolidTop[(int)Main.tile[num6 + 2, num7 + 1].type])
									{
										num8 = 3;
									}
									else
		...
		public static void Check2x2(int i, int j, int type)
		{
			if (!WorldGen.destroyObject)
			{
				...
				if (type == 138 && !WorldGen.SolidTile(num2, num3 + 2) && !WorldGen.SolidTile(num2 + 1, num3 + 2))
				{
					flag = true;
				}
				if (flag)
				{
					...
					{
						Projectile.NewProjectile((float)(num2 * 16) + 15.5f, (float)(num3 * 16 + 16), 0f, 0f, 99, 70, 10f, Main.myPlayer);
					}
					WorldGen.destroyObject = false;
					...
					if (!WorldGen.noTileActions)
					{
						if (!WorldGen.gen)
						{
							...
							if (num == 0)
							{
								if (Main.netMode == 0)
								{
									Main.NewText("你的世界被赐予了钴蓝矿！", 50, 255, 130);
								}
								else if (Main.netMode == 2)
								{
									NetMessage.SendData(25, -1, -1, "你的世界被赐予了钴蓝矿！", 255, 50f, 255f, 130f, 0);
								}
								num = 107;
								num3 *= 1.05f;
							}
							else if (num == 1)
							{
								if (Main.netMode == 0)
								{
									Main.NewText("你的世界被赐予了秘银矿！", 50, 255, 130);
								}
								else if (Main.netMode == 2)
								{
									NetMessage.SendData(25, -1, -1, "你的世界被赐予了秘银矿！", 255, 50f, 255f, 130f, 0);
								}
								num = 108;
							}
							else
							{
								if (Main.netMode == 0)
								{
									Main.NewText("你的世界被赐予了精金矿！", 50, 255, 130);
								}
								else if (Main.netMode == 2)
								{
									NetMessage.SendData(25, -1, -1, "你的世界被赐予了精金矿！", 255, 50f, 255f, 130f, 0);
								}
								num = 111;
							}
							int num5 = 0;
		...
		public static void CheckTree(int i, int j)
		{
			...
			if (Main.tile[i, j].frameX >= 22 && Main.tile[i, j].frameX <= 44 && Main.tile[i, j].frameY >= 132 && Main.tile[i, j].frameY <= 176)
			{
				...
				else if ((Main.tile[i, j].frameX != 22 || num4 != type) && (Main.tile[i, j].frameX != 44 || num5 != type))
				{
					WorldGen.KillTile(i, j, false, false, false);
				}
			}
			...
			else if (num2 != type && Main.tile[i, j].frameY < 198 && ((Main.tile[i, j].frameX != 22 && Main.tile[i, j].frameX != 44) || Main.tile[i, j].frameY < 132))
			{
				if (num4 == type || num5 == type)
				{
					if (num7 == type)
							...
							if (num8 < WorldGen.genRand.Next(11, 13))
							{
								...
								if (num6 == 0)
								{
									...
									else
									{
										...
										else if (num5 < WorldGen.genRand.Next(2, 8))
										{
			...
			if (!WorldGen.destroyObject)
			{
				...
				if (flag)
				{
					...
					else
					{
						...
						else if (num8 == 1 && Main.player[(int)Player.FindClosest(new Vector2((float)(i * 16), (float)(j * 16)), 16, 16)].statMana < Main.player[(int)Player.FindClosest(new Vector2((float)(i * 16), (float)(j * 16)), 16, 16)].statManaMax)
						{
		...
		public static void hitSwitch(int i, int j)
		{
			if (Main.tile[i, j] != null)
			{
				...
				else if (Main.tile[i, j].type == 132)
				{
					short num = 36;
					int num2 = (int)(Main.tile[i, j].frameX / 18 * -1);
					int num3 = (int)(Main.tile[i, j].frameY / 18 * -1);
					...
					for (int j = 0; j < WorldGen.numOutPump; j++)
					{
						...
						if (liquid2 < 255)
						{
							...
							if (lava == flag)
							{
								...
								Tile expr_109 = tile;
								expr_109.liquid += (byte)num5;
								Tile tile2 = Main.tile[num, num2];
								Tile expr_129 = tile2;
								expr_129.liquid -= (byte)num5;
				...
				if (Main.tile[i, j].wire)
				{
					...
					if (flag && Main.tile[i, j].active)
					{
						...
						else if (type == 42)
						{
							...
							Tile tile5 = Main.tile[i, num2];
							Tile expr_405 = tile5;
							expr_405.frameX += num3;
							Tile tile6 = Main.tile[i, num2 + 1];
							Tile expr_427 = tile6;
							expr_427.frameX += num3;
						...
						else if (type == 93)
						{
							...
							Tile tile7 = Main.tile[i, num4];
							Tile expr_4BD = tile7;
							expr_4BD.frameX += num5;
							Tile tile8 = Main.tile[i, num4 + 1];
							Tile expr_4DF = tile8;
							expr_4DF.frameX += num5;
							Tile tile9 = Main.tile[i, num4 + 2];
							Tile expr_501 = tile9;
							expr_501.frameX += num5;
						...
						else if (type == 126 || type == 100 || type == 95)
						{
							...
							Tile tile10 = Main.tile[num7, num6];
							Tile expr_5E7 = tile10;
							expr_5E7.frameX += num8;
							Tile tile11 = Main.tile[num7, num6 + 1];
							Tile expr_60A = tile11;
							expr_60A.frameX += num8;
							Tile tile12 = Main.tile[num7 + 1, num6];
							Tile expr_62D = tile12;
							expr_62D.frameX += num8;
							Tile tile13 = Main.tile[num7 + 1, num6 + 1];
							Tile expr_652 = tile13;
							expr_652.frameX += num8;
						...
						else if (type == 34 || type == 35 || type == 36)
						{
							...
							for (int m = num10; m < num10 + 3; m++)
							{
								...
								{
									Tile tile14 = Main.tile[m, n];
									Tile expr_756 = tile14;
									expr_756.frameX += num11;
									WorldGen.noWire(m, n);
								}
							}
						...
						else if (type == 33)
						{
							...
							Tile tile15 = Main.tile[i, j];
							Tile expr_7F6 = tile15;
							expr_7F6.frameX += num12;
						...
						else if (type == 92)
						{
							...
							Tile tile16 = Main.tile[i, num13];
							Tile expr_878 = tile16;
							expr_878.frameX += num14;
							Tile tile17 = Main.tile[i, num13 + 1];
							Tile expr_89A = tile17;
							expr_89A.frameX += num14;
							Tile tile18 = Main.tile[i, num13 + 2];
							Tile expr_8BC = tile18;
							expr_8BC.frameX += num14;
							Tile tile19 = Main.tile[i, num13 + 3];
							Tile expr_8DE = tile19;
							expr_8DE.frameX += num14;
							Tile tile20 = Main.tile[i, num13 + 4];
							Tile expr_900 = tile20;
							expr_900.frameX += num14;
							Tile tile21 = Main.tile[i, num13 + 5];
							Tile expr_922 = tile21;
							expr_922.frameX += num14;
						...
						else if (type == 142 || type == 143)
						{
							...
							if (type == 142)
							{
								for (int num18 = 0; num18 < 4; num18++)
								{
									...
									WorldGen.inPumpX[WorldGen.numInPump] = num19;
									WorldGen.inPumpY[WorldGen.numInPump] = num20;
									WorldGen.numInPump++;
								}
							}
							else
							{
								for (int num21 = 0; num21 < 4; num21++)
								{
									...
									WorldGen.outPumpX[WorldGen.numOutPump] = num22;
									WorldGen.outPumpY[WorldGen.numOutPump] = num23;
									WorldGen.numOutPump++;
								}
							}
						}
						else if (type == 105)
						{
							...
							else if (num26 == 41 && WorldGen.checkMech(i, j, 300))
							{
								int[] array2 = new int[10];
								int num33 = 0;
				...
				if (Main.tile[i, j].active)
				{
					...
					if (j < 1 || !Main.tile[i, j - 1].active || ((Main.tile[i, j - 1].type != 5 || Main.tile[i, j].type == 5) && (Main.tile[i, j - 1].type != 21 || Main.tile[i, j].type == 21) && (Main.tile[i, j - 1].type != 26 || Main.tile[i, j].type == 26) && (Main.tile[i, j - 1].type != 72 || Main.tile[i, j].type == 72) && (Main.tile[i, j - 1].type != 12 || Main.tile[i, j].type == 12)) || (Main.tile[i, j - 1].type == 5 && ((Main.tile[i, j - 1].frameX == 66 && Main.tile[i, j - 1].frameY >= 0 && Main.tile[i, j - 1].frameY <= 44) || (Main.tile[i, j - 1].frameX == 88 && Main.tile[i, j - 1].frameY >= 66 && Main.tile[i, j - 1].frameY <= 110) || Main.tile[i, j - 1].frameY >= 198)))
					{
						if (!effectOnly && !WorldGen.stopDrops)
						{
							...
							else if (Main.tileAlch[(int)Main.tile[i, j].type] || Main.tile[i, j].type == 32 || Main.tile[i, j].type == 51 || Main.tile[i, j].type == 52 || Main.tile[i, j].type == 61 || Main.tile[i, j].type == 62 || Main.tile[i, j].type == 69 || Main.tile[i, j].type == 71 || Main.tile[i, j].type == 73 || Main.tile[i, j].type == 74 || Main.tile[i, j].type == 113 || Main.tile[i, j].type == 115)
							{
								Main.PlaySound(6, i * 16, j * 16, 1);
							}
							...
							else if (Main.tile[i, j].type != 138)
							{
								Main.PlaySound(0, i * 16, j * 16, 1);
							}
						...
						for (int m = 0; m < num; m++)
						{
							...
							if (Main.tile[i, j].type == 129)
							{
								...
								else if (Main.tile[i, j].frameX == 18 || Main.tile[i, j].frameX == 72 || Main.tile[i, j].frameX == 126)
								{
									num5 = 69;
								}
								else
						...
						if (!effectOnly)
						{
							...
							else
							{
								...
								if (!noItem && !WorldGen.stopDrops && Main.netMode != 1)
								{
									...
									else if (Main.tile[i, j].type == 3 || Main.tile[i, j].type == 73)
									{
									...
									else if (Main.tile[i, j].type == 123)
									{
										num8 = 424;
									}
									else if (Main.tile[i, j].type == 124)
									{
										num8 = 480;
									}
									else if (Main.tile[i, j].type == 149)
									{
										if (Main.tile[i, j].frameX == 0 || Main.tile[i, j].frameX == 54)
										{
											num8 = 596;
										}
										else if (Main.tile[i, j].frameX == 18 || Main.tile[i, j].frameX == 72)
										{
											num8 = 597;
										}
										else if (Main.tile[i, j].frameX == 36 || Main.tile[i, j].frameX == 90)
										{
											num8 = 598;
										}
									}
									...
									else if (Main.tile[i, j].type == 19)
									{
										num8 = 94;
									}
									else if (Main.tile[i, j].type == 22)
									{
										num8 = 56;
									}
									...
									else if (Main.tile[i, j].type == 33)
									{
										num8 = 105;
									}
									else if (Main.tile[i, j].type == 37)
									{
										num8 = 116;
									}
									else if (Main.tile[i, j].type == 38)
									{
										num8 = 129;
									}
									else if (Main.tile[i, j].type == 39)
									{
										num8 = 131;
									}
									...
									else if (Main.tile[i, j].type == 41)
									{
										num8 = 134;
									}
									else if (Main.tile[i, j].type == 43)
									{
										num8 = 137;
									}
									else if (Main.tile[i, j].type == 44)
									{
										num8 = 139;
									}
									else if (Main.tile[i, j].type == 45)
									{
										num8 = 141;
									}
									else if (Main.tile[i, j].type == 46)
									{
										num8 = 143;
									}
									else if (Main.tile[i, j].type == 47)
									{
										num8 = 145;
									}
									else if (Main.tile[i, j].type == 48)
									{
										num8 = 147;
									}
									else if (Main.tile[i, j].type == 49)
									{
										num8 = 148;
									}
									else if (Main.tile[i, j].type == 51)
									{
										num8 = 150;
									}
									...
									else if (Main.tile[i, j].type == 54)
									{
										num8 = 170;
										Main.PlaySound(13, i * 16, j * 16, 1);
									}
									else if (Main.tile[i, j].type == 56)
									{
										num8 = 173;
									}
									else if (Main.tile[i, j].type == 57)
									{
										num8 = 172;
									}
									else if (Main.tile[i, j].type == 58)
									{
										num8 = 174;
									}
									...
									else if (Main.tile[i, j].type == 75)
									{
										num8 = 192;
									}
									else if (Main.tile[i, j].type == 76)
									{
										num8 = 214;
									}
									else if (Main.tile[i, j].type == 78)
									{
										num8 = 222;
									}
									else if (Main.tile[i, j].type == 81)
									{
										num8 = 275;
									}
									else if (Main.tile[i, j].type == 80)
									...
									else if (Main.tile[i, j].type == 107)
									{
										num8 = 364;
									}
									else if (Main.tile[i, j].type == 108)
									{
										num8 = 365;
									}
									else if (Main.tile[i, j].type == 111)
									{
										num8 = 366;
									}
									...
									else if (Main.tile[i, j].type == 116)
									{
										num8 = 408;
									}
									else if (Main.tile[i, j].type == 117)
									{
										num8 = 409;
									}
									else if (Main.tile[i, j].type == 129)
									{
										num8 = 502;
									}
									else if (Main.tile[i, j].type == 118)
									{
										num8 = 412;
									}
									else if (Main.tile[i, j].type == 119)
									{
										num8 = 413;
									}
									...
									else if (Main.tile[i, j].type == 121)
									{
										num8 = 415;
									}
									else if (Main.tile[i, j].type == 122)
									{
										num8 = 416;
									}
									else if (Main.tile[i, j].type == 136)
									...
									else if (Main.tile[i, j].type == 137)
									{
										num8 = 539;
									}
									else if (Main.tile[i, j].type == 141)
									{
										num8 = 580;
									}
									else if (Main.tile[i, j].type == 145)
									{
										num8 = 586;
									}
									else if (Main.tile[i, j].type == 146)
									{
										num8 = 591;
									}
									else if (Main.tile[i, j].type == 147)
									{
										num8 = 593;
									}
									else if (Main.tile[i, j].type == 148)
									{
										num8 = 594;
									}
									else if (Main.tile[i, j].type == 135)
									{
										if (Main.tile[i, j].frameY == 0)
										{
											num8 = 529;
									...
									else if (Main.tile[i, j].type == 131)
									{
										num8 = 512;
									}
									else if (Main.tile[i, j].type == 61 || Main.tile[i, j].type == 74)
									{
										if (Main.tile[i, j].frameX == 144)
										{
											Item.NewItem(i * 16, j * 16, 16, 16, 331, WorldGen.genRand.Next(2, 4), false, 0);
										}
										else if (Main.tile[i, j].frameX == 162)
										{
											num8 = 223;
										}
										else if (Main.tile[i, j].frameX >= 108 && Main.tile[i, j].frameX <= 126 && WorldGen.genRand.Next(100) == 0)
										{
											num8 = 208;
										}
									...
									else if (Main.tile[i, j].type == 59 || Main.tile[i, j].type == 60)
									{
										num8 = 176;
									}
									else if (Main.tile[i, j].type == 71 || Main.tile[i, j].type == 72)
									{
										if (WorldGen.genRand.Next(50) == 0)
										{
											num8 = 194;
									...
									else if (Main.tile[i, j].type >= 63 && Main.tile[i, j].type <= 68)
									{
									...
									else if (Main.tileAlch[(int)Main.tile[i, j].type] && Main.tile[i, j].type > 82)
									{
			...
			if (Main.hardMode)
			{
				...
				if (type == 23 || type == 25 || type == 32 || type == 112)
				{
					...
					while (flag)
					{
						...
						else if (Main.tile[num6, num7].type == 59)
						{
							if (WorldGen.genRand.Next(2) == 0)
							{
								flag = true;
						...
						else if (Main.tile[num6, num7].type == 60)
						{
							if (WorldGen.genRand.Next(2) == 0)
							{
								flag = true;
						...
						else if (Main.tile[num6, num7].type == 69)
						{
							if (WorldGen.genRand.Next(2) == 0)
							{
								flag = true;
		...
		public static void UpdateWorld()
		{
			...
			while ((float)num3 < (float)(Main.maxTilesX * Main.maxTilesY) * num)
			{
				...
				if (Main.tile[num4, num5] != null)
				{
					...
					else if (Main.tile[num4, num5].active)
					{
						...
						else if (Main.tile[num4, num5].type == 116 || Main.tile[num4, num5].type == 112)
						{
						...
						else if (Main.tile[num4, num5].type == 78)
						{
							if (!Main.tile[num4, num8].active)
							{
						...
						else if (Main.tile[num4, num5].type == 2 || Main.tile[num4, num5].type == 23 || Main.tile[num4, num5].type == 32 || Main.tile[num4, num5].type == 109)
						{
						...
						else if (Main.tile[num4, num5].type == 20 && WorldGen.genRand.Next(20) == 0 && !WorldGen.PlayerLOS(num4, num5))
						{
					...
					if (Main.tile[num4, num5].active)
					{
						...
						if (Main.tile[num4, num5].type == 60)
						{
							...
							else if (WorldGen.genRand.Next(500) == 0 && (!Main.tile[num4, num8].active || Main.tile[num4, num8].type == 61 || Main.tile[num4, num8].type == 74 || Main.tile[num4, num8].type == 69) && !WorldGen.PlayerLOS(num4, num5))
							{
								WorldGen.GrowTree(num4, num5);
							}
							bool flag5 = false;
			...
			for (int i = 1; i < Main.maxTilesX - 1; i++)
			{
				for (int j = Main.maxTilesY - 2; j > 0; j--)
				{
					...
					else if (Main.tile[i, j].liquid > 0)
					{
						...
						else if ((!Main.tile[i + 1, j].active || !Main.tileSolid[(int)Main.tile[i + 1, j].type] || Main.tileSolidTop[(int)Main.tile[i + 1, j].type]) && Main.tile[i + 1, j].liquid != Main.tile[i, j].liquid)
						{
							Liquid.AddWater(i, j);
						}
						if (Main.tile[i, j].lava)
		...
		public static void EveryTileFrame()
		{
			...
			{
				float num = (float)i / (float)Main.maxTilesX;
				Main.statusText = "搜寻物块结构: " + (int)(num * 100f + 1f) + "%";
				for (int j = 0; j < Main.maxTilesY; j++)
				{
					if (Main.tile[i, j].active)
					{
						...
						if (!noBreak || !Main.tileFrameImportant[(int)Main.tile[i, j].type] || Main.tile[i, j].type == 4)
						{
							...
							if (Main.tileFrameImportant[(int)Main.tile[i, j].type])
							{
								if (num == 4)
								{
									...
									if (Main.tile[i, j - 1] != null && Main.tile[i, j - 1].active)
									{
										byte type = Main.tile[i, j - 1].type;
									}
									if (Main.tile[i, j + 1] != null && Main.tile[i, j + 1].active)
									{
								...
								else if (num == 136)
								{
									...
									if (Main.tile[i, j - 1] != null && Main.tile[i, j - 1].active)
									{
										byte type2 = Main.tile[i, j - 1].type;
									}
									if (Main.tile[i, j + 1] != null && Main.tile[i, j + 1].active)
									{
								...
								else if (num == 3 || num == 24 || num == 61 || num == 71 || num == 73 || num == 74 || num == 110 || num == 113)
								{
								...
								else if (num == 34 || num == 35 || num == 36 || num == 106)
								{
								...
								else if (num == 14 || num == 17 || num == 26 || num == 77 || num == 86 || num == 87 || num == 88 || num == 89 || num == 114 || num == 133)
								{
									WorldGen.Check3x2(i, j, (int)((byte)num));
								}
								else if (num == 135 || num == 144 || num == 141)
								{
								...
								else if (num == 13 || num == 33 || num == 50 || num == 78)
								{
								...
								else if (num == 132 || num == 138 || num == 142 || num == 143)
								{
								...
								else if (num == 85 || num == 94 || num == 95 || num == 96 || num == 97 || num == 98 || num == 99 || num == 100 || num == 125 || num == 126)
								{
									WorldGen.Check2x2(i, j, num);
								}
								else if (num == 81)
								...
								else if (num == 72)
								{
									...
									else if (num27 != num && Main.tile[i, j].frameX == 0)
									{
										Main.tile[i, j].frameNumber = (byte)WorldGen.genRand.Next(3);
										if (Main.tile[i, j].frameNumber == 0)
										{
								...
								if (!WorldGen.noTileActions && (num == 53 || num == 112 || num == 116 || num == 123))
								{
									if (Main.netMode == 0)
									{
										if (Main.tile[i, j + 1] != null && !Main.tile[i, j + 1].active)
										{
											...
											if (flag5)
											{
												...
												Main.tile[i, j].active = false;
												int num38 = Projectile.NewProjectile((float)(i * 16 + 8), (float)(j * 16 + 8), 0f, 0.41f, type3, 10, 0f, Main.myPlayer);
									...
									else if (Main.netMode == 2 && Main.tile[i, j + 1] != null && !Main.tile[i, j + 1].active)
									{
										...
										if (flag6)
										{
											...
											Main.tile[i, j].active = false;
											int num39 = Projectile.NewProjectile((float)(i * 16 + 8), (float)(j * 16 + 8), 0f, 2.5f, type4, 10, 0f, Main.myPlayer);
											Main.projectile[num39].velocity.Y = 0.5f;
											Projectile projectile = Main.projectile[num39];
