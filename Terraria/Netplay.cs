		...
		public static void ServerLoop(object threadContext)
		{
			...
			while (!Netplay.disconnect)
			{
				...
				if (!WorldGen.saveLock && !Main.dedServ)
				{
					if (num3 == 0)
					{
						Main.statusText = "等待客户端连接...";
					}
					else
					{
						Main.statusText = num3 + " 个玩家已加入";
					}
				}
				if (num3 == 0)
				{
			...
			while (!Netplay.disconnect && !Netplay.stopListen)
			{
				...
				if (num >= 0)
				{
					try
					{
						...
						Console.WriteLine(Netplay.serverSock[num].tcpClient.Client.RemoteEndPoint + " 正在连接中...");
						continue;
					}
					catch (Exception ex)
					{
