...
namespace Terraria
{
	public class Liquid
	{
		...
		public static double QuickWater(int verbose = 0, int minY = -1, int maxY = -1)
		{
			...
			for (int i = maxY; i >= minY; i--)
			{
				if (verbose > 0)
				{
					...
					num2 /= (float)verbose;
					Main.statusText = "稳定液体水平面: " + (int)(num2 * 100f + 1f) + "%";
				}
				else if (verbose < 0)
				{
					float num3 = (float)(maxY - i) / (float)(maxY - minY + 1);
					num3 /= -(float)verbose;
					Main.statusText = "创建地下世界: " + (int)(num3 * 100f + 1f) + "%";
				}
				for (int j = 0; j < 2; j++)
				{
					int num4 = 2;
						...
						if (Main.tile[num7, i].liquid > 0)
						{
							...
							while (flag3 && num9 > 3 && num9 < Main.maxTilesX - 3 && num10 < Main.maxTilesY - 3)
							{
								...
								if (num11 == 0)
								{
									...
									else if (Main.tile[num9 - num8, num10].liquid == 0 && (!Main.tile[num9 - num8, num10].active || !Main.tileSolid[(int)Main.tile[num9 - num8, num10].type] || Main.tileSolidTop[(int)Main.tile[num9 - num8, num10].type]))
									{
										num11 = -num8;
									}
								}
			...
			else
			{
				...
				else
				{
					...
					if ((!Main.tile[this.x, this.y + 1].active || !Main.tileSolid[(int)Main.tile[this.x, this.y + 1].type] || Main.tileSolidTop[(int)Main.tile[this.x, this.y + 1].type]) && (Main.tile[this.x, this.y + 1].liquid <= 0 || Main.tile[this.x, this.y + 1].lava == Main.tile[this.x, this.y].lava) && Main.tile[this.x, this.y + 1].liquid < 255)
					{
						...
						Tile expr_4A0 = tile2;
						expr_4A0.liquid -= (byte)num;
						Tile tile3 = Main.tile[this.x, this.y + 1];
						Tile expr_4CB = tile3;
						expr_4CB.liquid += (byte)num;
					...
					else if (Main.tile[this.x, this.y].liquid == 254 && liquid == 255)
					{
						Main.tile[this.x, this.y].liquid = 255;
						this.kill++;
					}
		...
		public static void DelWater(int l)
		{
			...
			else if (Main.tile[num, num2 + 1].liquid < 255 && (!Main.tile[num, num2 + 1].active || !Main.tileSolid[(int)Main.tile[num, num2 + 1].type] || Main.tileSolidTop[(int)Main.tile[num, num2 + 1].type]) && !Liquid.stuck)
			{
