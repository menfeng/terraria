			...
			if (Main.showItemText)
			{
				if (newItem.name != null && newItem.active)
				{
					if (Main.netMode != 2)
					{
						for (int i = 0; i < 20; i++)
						{
							if (Main.itemText[i].active && Main.itemText[i].name == newItem.CName)
							{
								...
								vector = Main.fontMouseText.MeasureString(str);
						...
						if (num >= 0)
						{
							string text2 = newItem.AffixName();
							if (stack > 1)
							{
								object obj2 = text2;
